...
-- Comentario que indica que a continuación se definirá la estructura de la tabla lote
...
CREATE TABLE `lote` (
  ...
  -- Referencia al producto asociado a este lote

  `peso_total` decimal(10,0) DEFAULT NULL,
  -- Peso total del lote

  `humedad` decimal(10,0) DEFAULT NULL,
  -- Porcentaje de humedad del lote

  `fermentacion` decimal(10,0) DEFAULT NULL,
  -- Porcentaje de fermentación del lote

  `fecha_cosecha` date DEFAULT NULL,
  -- Fecha en que se cosechó el lote

  `id_pedido` int DEFAULT NULL,
  -- Referencia al pedido asociado a este lote

  PRIMARY KEY (`idLote`),
  -- Define la clave primaria de la tabla

  KEY `id_producto_idx` (`id_producto`),
  -- Índice para optimizar consultas por id_producto

  KEY `id_pedido_idx` (`id_pedido`),
  -- Índice para optimizar consultas por id_pedido

  CONSTRAINT `id_pedido` FOREIGN KEY (`id_pedido`) REFERENCES `pedido` (`id_Pedido`),
  -- Llave foránea que relaciona id_pedido con la tabla pedido

  CONSTRAINT `id_producto` FOREIGN KEY (`id_producto`) REFERENCES `producto` (`id_Producto`)
  -- Llave foránea que relaciona id_producto con la tabla producto
...
-- Sección para insertar los datos de la tabla lote
