...
-- Comentario que indica que a continuación se definirá la estructura de la tabla pedido
...
CREATE TABLE `pedido` (
  ...
  -- Referencia al proveedor que realiza el pedido

  `fecha_creacion` date DEFAULT NULL,
  -- Fecha de creación del pedido

  `recibido` tinyint DEFAULT NULL,
  -- Indica si el pedido fue recibido (1 = sí, 0 = no)

  `observaciones` varchar(45) DEFAULT NULL,
  -- Comentarios u observaciones del pedido

  PRIMARY KEY (`id_Pedido`),
  -- Define la clave primaria de la tabla

  KEY `id_proveedor_idx` (`id_proveedor`),
  -- Índice para optimizar consultas por id_proveedor

  CONSTRAINT `id_proveedor` FOREIGN KEY (`id_proveedor`) REFERENCES `proveedor` (`id_Proveedor`)
  -- Llave foránea que relaciona id_proveedor con la tabla proveedor
...
-- Sección para insertar los datos de la tabla pedido
